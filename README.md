...
Every `.sh` script has a `.ps1` equivalent, for building on Windows with PowerShell instead of bash — e.g. `_preProcessAndCheckAll.ps1` alongside `_preProcessAndCheckAll.sh`. Both take the same arguments and behave the same way; the examples below use the `.sh` form but can be replaced with `.ps1`.
...
# on Windows: ./_commitToMainRepos.ps1
```
...
| `_commitToMainRepos.sh` / `.ps1` | Publishes the generated IGs to the `base` and `base-r5` repositories. |
...
(`_commitToMainRepos.ps1` on Windows.)
